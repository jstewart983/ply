...
describe('Plyr API:', function () {
  ...
  describe('GET /api/plyrs', function () {
    ...
    beforeEach(function (done) {
      request(app).get('/api/plyrs').expect(200).expect('Content-Type', /json/).end(function (err, res) {
        if (err) {
          return done(err);
        }
        plyrs = res.body;
  ...
  describe('POST /api/plyrs', function () {
    beforeEach(function (done) {
      request(app).post('/api/plyrs').send({
        name: 'New Plyr',
  ...
  describe('GET /api/plyrs/:id', function () {
    ...
    beforeEach(function (done) {
      request(app).get('/api/plyrs/' + newPlyr._id).expect(200).expect('Content-Type', /json/).end(function (err, res) {
        if (err) {
          return done(err);
        }
        plyr = res.body;
  ...
  describe('PUT /api/plyrs/:id', function () {
    ...
    beforeEach(function (done) {
      request(app).put('/api/plyrs/' + newPlyr._id).send({
        name: 'Updated Plyr',
  ...
  describe('DELETE /api/plyrs/:id', function () {
    ...
    it('should respond with 204 on successful removal', function (done) {
      request(app)['delete']('/api/plyrs/' + newPlyr._id).expect(204).end(function (err, res) {
        if (err) {
          return done(err);
        }
        done();
    ...
    it('should respond with 404 when plyr does not exist', function (done) {
      request(app)['delete']('/api/plyrs/' + newPlyr._id).expect(404).end(function (err, res) {
        if (err) {
          return done(err);
        }
        done();
...
//# sourceMappingURL=plyr.integration.js.map
