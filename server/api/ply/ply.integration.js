...
describe('Ply API:', function () {
  ...
  describe('GET /api/plys', function () {
    ...
    beforeEach(function (done) {
      request(app).get('/api/plys').expect(200).expect('Content-Type', /json/).end(function (err, res) {
        if (err) {
          return done(err);
        }
        plys = res.body;
  ...
  describe('POST /api/plys', function () {
    beforeEach(function (done) {
      request(app).post('/api/plys').send({
        name: 'New Ply',
  ...
  describe('GET /api/plys/:id', function () {
    ...
    beforeEach(function (done) {
      request(app).get('/api/plys/' + newPly._id).expect(200).expect('Content-Type', /json/).end(function (err, res) {
        if (err) {
          return done(err);
        }
        ply = res.body;
  ...
  describe('PUT /api/plys/:id', function () {
    ...
    beforeEach(function (done) {
      request(app).put('/api/plys/' + newPly._id).send({
        name: 'Updated Ply',
  ...
  describe('DELETE /api/plys/:id', function () {
    ...
    it('should respond with 204 on successful removal', function (done) {
      request(app)['delete']('/api/plys/' + newPly._id).expect(204).end(function (err, res) {
        if (err) {
          return done(err);
        }
        done();
    ...
    it('should respond with 404 when ply does not exist', function (done) {
      request(app)['delete']('/api/plys/' + newPly._id).expect(404).end(function (err, res) {
        if (err) {
          return done(err);
        }
        done();
...
//# sourceMappingURL=ply.integration.js.map
